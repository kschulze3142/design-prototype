...
function DocPreview({ title, from, to, pages }: { title: string; from: string; to: string; pages: number }) {
  return (
    <div className="rounded-2xl overflow-hidden border border-slate-200" style={{ background: 'repeating-linear-gradient(45deg, rgba(15,118,110,0.03) 0px, rgba(15,118,110,0.03) 1px, transparent 1px, transparent 8px)' }}>
      <div className="p-5" style={{ background: 'linear-gradient(135deg, rgba(204,251,241,0.7), rgba(240,253,250,0.5))' }}>
...
function TemplateCard({ t, onClick, selected }: { t: Template; onClick: () => void; selected: boolean }) {
  return (
    <button onClick={onClick}
      className={`w-full text-left rounded-[24px] p-1 transition ${selected ? 'ring-2 ring-[var(--accent)] ring-offset-2' : ''}`}>
