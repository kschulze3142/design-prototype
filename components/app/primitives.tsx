...
export const AppButton = ({ variant = 'primary', size = 'md', className = '', children, icon, iconRight, ...rest }: {
  ...
  const variants = {
    primary: 'text-white shadow-[0_6px_16px_-8px_var(--accent),inset_0_1px_0_rgba(255,255,255,0.18)]',
  ...
  return (
    <button className={`${base} ${sizes[size]} ${variants[variant]} ${className}`}
      style={variant === 'primary' ? { background: 'var(--accent)' } : undefined} {...rest}>
...
// Avatar
export const Avatar = ({ name, size = 32, tone = 'teal' }: { name: string; size?: number; tone?: string }) => {
  const init = name.split(' ').map(p => p[0]).slice(0, 2).join('').toUpperCase();
  const t = STATUS_TONES[tone] || STATUS_TONES.teal;
  return (
...
export const StatCard = ({ label, value, helper, trend, icon, className = '' }: {
  ...
  <Card className={`p-6 flex flex-col gap-3 ${className}`}>
    <div className="flex items-center justify-between">
      ...
      {icon && <span className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: 'oklch(0.96 0.04 var(--accent-h))', color: 'var(--accent-deep)' }}>{icon}</span>}
    </div>
